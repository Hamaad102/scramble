...
import { MainButton, SmallButton, SettingsButton } from '../styles/size'

const Home: NextPage = () => {
	// Modal Open/Close
	const [openHelp, setHelpOpen] = useState<boolean>(false)
	const handleHelp = () => setHelpOpen(true)
	const handleHelpClose = () => setHelpOpen(false)

	const [openSettings, setSettingsOpen] = useState<boolean>(false)
		...
		<ThemeProvider theme={darkMode ? dark : light}>
			<div>
				<Head>
					...
					<meta
						name='description'
						content='Play your favorite word game against your friends. Compete to see who can get the highest score within the time limit.'
					/>
					<link
								...
								<Stack spacing={3} alignItems='center'>
									<motion.div variants={item}>
										<Link href='/create' passHref>
											<Button sx={{ ...MainButton, ...ButtonColors }}>
												Create Lobby
											</Button>
										</Link>
									</motion.div>
									...
									<motion.div variants={item}>
										<Stack direction='row' justifyContent='center' spacing={2}>
											<Button
												sx={{ ...SmallButton, ...ButtonColors }}
												onClick={handleHelp}
											>
												?
											</Button>
											<Button
												sx={{ ...SettingsButton, ...ButtonColors }}
