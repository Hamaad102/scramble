...
const Game: NextPage = () => {
	...
	const keyDownHandler = (event: KeyboardEvent<HTMLInputElement>) => {
		if (event.key.length === 1) {
			const regex = /[a-zA-z]+/
			if (regex.test(event.key)) updateWord(event.key.toUpperCase())
		} else {
	...
	useEffect(() => {
		// If user refreshes or tries to join the game after the fact they won't be able to
		if (targetWords.length === 0) {
			updateJoinError('Unable to join. Game already in session.')
			router.push(`/join`)
		} else {
			setLoading(false)
			switch (timeLimit) {
	...
	// Three second countdown for game
	useEffect(() => {
		if (timer !== 0 && !loading) {
				...
				<Container sx={{ height: '100%' }}>
					{loading ? (
						...
						<Stack mt='30vh' alignItems='center'>
							<Typography
								variant='h4'
								sx={{ color: 'white', fontSize: '15rem!important' }}
							>
								{timer}
							</Typography>
						</Stack>
						...
						>
							...
							<Box
								sx={{
									width: { sm: '23rem', xs: '76vw' },
									marginTop: { sm: '50px!important', xs: '40px!important' },
								}}
							>
								<LinearProgress
									sx={{
