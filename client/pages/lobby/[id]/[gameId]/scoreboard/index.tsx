...
// Style
import { SmallButton } from '../../../../../styles/size'
import { AltButtonColors, PlayAgain } from '../../../../../styles/colors'
...
const Scoreboard: NextPage = () => {
	...
	const { score, opponent, opponentScore, playAgain } = useGame()
	const {
		lobbyName,
		transition,
		updateTransition,
	...
	useEffect(() => {
		if (opponentScore === -1) {
			if (opponent === '') setDisconnected(true)
		}
	}, [opponent])

	const redirect = () => {
		updateJoinError('Opponent disconnected.')
		exitLobby(lobbyName)
		router.push(`/join`)
	}
	...
	return (
		<ThemeProvider theme={darkMode ? dark : light}>
			<Head>
				<title>Scramble - Scoreboard</title>
				<link
				...
				<Container sx={{ height: '100%' }}>
					...
					{disconnected ? (
						...
						<Stack mt='30vh' alignItems='center'>
							<CircularProgress sx={{ color: 'secondary.main' }} size={140} />
							<Typography
								variant='h4'
								sx={{
									marginTop: 10,
									color: 'white',
									fontSize: { sm: '2rem!important', xs: '1rem!important' },
								}}
							>
						...
						>
							...
							<Stack direction='row' spacing={{ sx: 16, xs: 10 }}>
							...
							<Stack direction='row' spacing={3}>
								...
								>
									<span className='butText'>Play Again</span>
								</Button>
							</Stack>
						</Stack>
					)}
